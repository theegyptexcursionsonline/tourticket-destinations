...
export function assertStripePaymentAvailableForBooking(paymentIntent: Stripe.PaymentIntent): void {
  if (paymentIntent.status !== 'succeeded') {
    throw new Error('Payment processing failed. Payment has not been completed. Please complete the payment and try again.');
  }

  const charge = paymentIntent.latest_charge;
  if (!charge || typeof charge === 'string') {
    throw new Error('Payment processing failed. Payment status could not be verified. Please restart checkout.');
  }
  ...
  if (charge.refunded || Number(charge.amount_refunded || 0) > 0) {
    throw new Error('Payment processing failed. This payment was refunded and cannot be used to create a booking. Please restart checkout.');
  }
}
