...
const CustomerTypeSelector = ({
  ...
  return (
    <div className="bg-white p-4 sm:p-6 shadow-xl space-y-3 sm:space-y-4 border-t border-b border-e border-slate-200">
      <h2 className="text-lg sm:text-xl font-bold text-slate-900 mb-3 sm:mb-4">{t('checkout.yourDetails')}</h2>
      ...
      <div className="space-y-3">
        ...
        <motion.button
          ...
          className={`w-full p-3 sm:p-4 border-2 rounded-xl text-start transition-all ${
            customerType === 'guest'
        ...
        <motion.button
          ...
          className="w-full p-3 sm:p-4 border-2 border-slate-200 rounded-xl text-start transition-all hover:border-blue-300 hover:bg-blue-50"
          whileHover={{ scale: 1.01 }}
          whileTap={{ scale: 0.99 }}
        >
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-semibold text-sm sm:text-base text-slate-800">{t('auth.login')}</div>
                <div className="text-xs sm:text-sm text-slate-600 truncate sm:whitespace-normal">{t('auth.loginDescription')}</div>
              </div>
            </div>
            <div className="text-blue-500 text-xs sm:text-sm font-medium flex-shrink-0 hidden xs:block">{t('auth.login')} →</div>
          </div>
        </motion.button>
        ...
        <motion.button
          ...
          className="w-full p-3 sm:p-4 border-2 border-slate-200 rounded-xl text-start transition-all hover:border-green-300 hover:bg-green-50"
          whileHover={{ scale: 1.01 }}
          whileTap={{ scale: 0.99 }}
        >
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-semibold text-sm sm:text-base text-slate-800">{t('auth.createAccount')}</div>
                <div className="text-xs sm:text-sm text-slate-600 truncate sm:whitespace-normal">{t('auth.signUpDescription')}</div>
              </div>
            </div>
  ...
  {item.selectedBookingOption && (
    <p className="text-sm text-blue-600 font-medium mb-1">{item.selectedBookingOption.title}</p>
        ...
        {item.selectedAddOns && item.selectedAddOnDetails && Object.keys(item.selectedAddOns).length > 0 && (
          <div className="text-xs text-slate-500 mt-1 ps-4 border-s-2 border-slate-200">
      ...
      <div className="text-end flex flex-col justify-between items-end flex-shrink-0">
...
const BookingSummary = ({ pricing, promoCode, setPromoCode, applyPromoCode, isProcessing, isApplyingCoupon, couponMessage }: any) => {
  ...
  return (
    <aside className="bg-white border-t border-b border-s border-slate-200 p-4 sm:p-6 shadow-xl lg:sticky lg:top-28">
      <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-4">{t('checkout.orderSummary')}</h3>
      ...
      <div className="mt-4 p-3 sm:p-4 bg-slate-50 border-t border-b border-slate-200">
        <div className="flex justify-between text-xs sm:text-sm text-slate-600"><span>{t('checkout.subtotal')}</span><span>{formatPrice(pricing.subtotal)}</span></div>
        ...
        <div className="flex justify-between text-xs sm:text-sm text-slate-600 mt-1.5 sm:mt-2"><span>{t('booking.taxes')}</span><span>{formatPrice(pricing.tax)}</span></div>
        {pricing.discount > 0 && (
        ...
        <div className="border-t pt-2 sm:pt-3 mt-2 sm:mt-3 flex justify-between items-center">
          <p className="text-xs sm:text-sm text-slate-600">{t('checkout.total')}</p>
          <p className="text-base sm:text-lg font-bold text-rose-600">{formatPrice(pricing.total)}</p>
        </div>
      </div>

      <div className="mt-4">
        <label className="block text-sm font-medium text-slate-700 mb-2 sm:hidden">{t('checkout.promoCode')}</label>
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          >
            {isApplyingCoupon ? <Loader2 className="animate-spin" size={20} /> : t('checkout.applyPromo')}
          </button>
        </div>
        {couponMessage && (
...
const CheckoutFormStep = ({
  ...
  return (
    <form onSubmit={handleSubmit} id="checkout-form" className="bg-white p-4 sm:p-6 md:p-8 shadow-xl space-y-6 sm:space-y-8 border-t border-b border-e border-slate-200">
      ...
      {user && (
        <section className="bg-green-50 border border-green-200 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-green-800">{t('auth.welcomeBack')}, {user.firstName}!</h3>
              <p className="text-sm text-green-600">{t('auth.loginDescription')}</p>
            </div>
          </div>
        </section>
      )}
      ...
      {(customerType === 'guest' || user) && (
        <section>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
            <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900">{t('checkout.personalInfo')}</h2>
      ...
      {(customerType === 'guest' || user) && (
        <section>
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">{t('checkout.specialRequests')}</label>
              <textarea 
    ...
    <div className="relative">
      ...
      >
        ...
        >
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-2">{t('checkout.bookingComplete')}</h1>
          <p className="text-slate-500">{t('checkout.bookingCompleteDescription')}</p>
        </motion.div>
        ...
        >
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl shadow-slate-200/50 overflow-hidden border border-slate-100">
            ...
            <div className="relative flex items-center px-0">
              <div className="w-6 h-12 bg-slate-100 rounded-e-full -ms-3" />
              <div className="flex-1 border-t-2 border-dashed border-slate-200 mx-2" />
              <div className="w-6 h-12 bg-slate-100 rounded-s-full -me-3" />
            </div>
            ...
            <div className="px-6 py-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div>
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">{t('checkout.bookingRef')}</p>
...
const TrustIndicators = () => {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-6 md:gap-12 py-6 sm:py-8 mt-8 sm:mt-12 border-t border-slate-200">
      <div className="flex items-center gap-2 text-sm sm:text-base text-slate-600"><Shield size={18} className="sm:w-5 sm:h-5 text-emerald-600" /><span>{t('checkout.securePayment')}</span></div>
      <div className="flex items-center gap-2 text-sm sm:text-base text-slate-600"><Smartphone size={18} className="sm:w-5 sm:h-5 text-red-600" /><span>{t('tourCard.mobileTicket')}</span></div>
      <div className="flex items-center gap-2 text-sm sm:text-base text-slate-600"><Headphones size={18} className="sm:w-5 sm:h-5 text-slate-900" /><span>{t('tour.support247')}</span></div>
    </div>
  );
};
 ...
 const pricing = useMemo(() => {
  ...
  return (
    <>
      ...
      {showMobileStickyCTA && (
        <div className="lg:hidden fixed bottom-0 start-0 end-0 bg-white/95 backdrop-blur-sm border-t border-slate-200 px-4 pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] z-50 shadow-[0_-6px_20px_rgba(15,23,42,0.15)]">
          ...
          >
            {isProcessing ? <Loader2 className="animate-spin" size={20} /> : <span className="inline-flex items-center justify-center gap-2"><Lock size={16} /> {t('checkout.completeBooking')}</span>}
          </button>
        </div>
      )}
