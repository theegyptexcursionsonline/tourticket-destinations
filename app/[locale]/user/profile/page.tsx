...
const ProfileForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: data.error || t('common.somethingWentWrong') });
      }
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-2">
          <FileText size={16} className="inline me-1" />
          Bio
        </label>
        <textarea
          name="bio"
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100">
        <div className="border-b border-slate-100">
          <nav className="flex px-4 sm:px-6">
            ...
            >
              ...
              <span className="whitespace-nowrap">{t('user.personalInfo')}</span>
            </button>
            <button
            ...
            >
              ...
              <span className="whitespace-nowrap">{t('user.changePassword')}</span>
            </button>
          </nav>
        </div>
        ...
        <div className="p-4 sm:p-6">
          {activeTab === 'profile' && (
            <div>
              <h2 className="text-lg sm:text-xl font-semibold text-slate-900 mb-4 sm:mb-6">{t('user.editProfile')}</h2>
              <ProfileForm />
            </div>
          )}
          ...
          {activeTab === 'password' && (
            <div>
              <h2 className="text-lg sm:text-xl font-semibold text-slate-900 mb-2">{t('user.changePassword')}</h2>
