    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-red-200 overflow-hidden p-6">
        <div className="flex items-center gap-3 text-red-600">
          ...
          <div>
            <h3 className="font-semibold">{t('common.notFound')}</h3>
            <p className="text-sm text-red-500">{t('common.noData')}</p>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="relative w-full h-48 sm:w-64 sm:h-40 flex-shrink-0">
          ...
          />
          <div className={`absolute top-3 end-3 px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(booking.status)}`}>
            {booking.status}
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex items-center text-slate-600">
                  <Calendar size={16} className="me-2 text-slate-400 flex-shrink-0" />
                  <span className="truncate">
                    {formatDisplayDate(booking.bookingDate)}
                  </span>
                </div>
                <div className="flex items-center text-slate-600">
                  <Clock size={16} className="me-2 text-slate-400 flex-shrink-0" />
                  <span className="truncate">{booking.bookingTime}</span>
                </div>
                <div className="flex items-center text-slate-600">
                  <Users size={16} className="me-2 text-slate-400 flex-shrink-0" />
                  <span className="truncate">{formatGuestBreakdown()}</span>
                </div>
                {booking.tour.rating && (
                  <div className="flex items-center text-slate-600">
                    <Star size={16} className="me-1 text-yellow-400 fill-yellow-400 flex-shrink-0" />
            ...
            <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-3 sm:gap-2 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-200">
              <div className="text-end">
                <div className="text-xs text-slate-500 mb-1">{t('price.totalPrice')}</div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 py-12 px-4">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-slate-900 mb-2">{t('user.myBookings')}</h1>
          <p className="text-slate-600">
            {bookings.length > 0
              ? `${bookings.length} ${t('user.myBookings').toLowerCase()}`
              : t('user.noBookings')
            }
          </p>
        </div>
        ...
        {bookings.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-slate-900 mb-2">{t('user.noBookings')}</h3>
