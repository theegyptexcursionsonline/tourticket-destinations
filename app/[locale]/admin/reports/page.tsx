...
const StarRating = ({ rating, size = 'sm' }: { rating: number; size?: 'sm' | 'lg' }) => (
  <div className="flex items-center gap-1">
    ...
    <span className={`font-semibold text-slate-700 ${size === 'lg' ? 'text-lg ms-2' : 'text-sm ms-1'}`}>
      {rating.toFixed(2)}
    </span>
  </div>
);
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 ps-3 pe-3 py-2 border border-slate-300 rounded-xl hover:bg-slate-50 transition-colors bg-white min-w-[180px]"
            >
              <Calendar className="w-4 h-4 text-slate-500" />
              <span className="text-sm font-medium text-slate-700 truncate flex-1 text-start">
                {getDateRangeLabel()}
              </span>
            ...
            {showDatePicker && (
              <>
                ...
                {/* Dropdown Panel */}
                <div className="absolute end-0 top-full mt-2 z-50 bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden w-80">
                  {/* Preset Options */}
                    ...
                    <div className="grid grid-cols-2 gap-1">
                      {DATE_RANGE_PRESETS.filter(p => p.value !== 'custom').map((preset) => (
                        <button
                          ...
                          className={`px-3 py-2 text-sm rounded-lg text-start transition-colors ${
                        ...
                        >
                          {preset.label}
                          {dateRange === preset.value && !appliedCustomRange && (
                            <Check className="inline-block w-4 h-4 ms-1 text-sky-600" />
                          )}
                        </button>
                      ))}
                    </div>
