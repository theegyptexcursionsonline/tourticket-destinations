    ...
    <Link href={`/destinations/${destination.slug}`} className="group block bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center text-sm text-slate-500 mt-1">
          ...
          <span>{t('destinations.toursAvailable', { count: destination.tourCount })}</span>
        </div>
      </div>
    </Link>
  );
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="text-center mb-12">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900">{t('destinations.exploreAll')}</h1>
        ...
        <div className="mt-8 max-w-2xl mx-auto">
              ...
              <div className="relative bg-white/95 backdrop-blur-xl rounded-full shadow-xl hover:shadow-2xl border-2 border-blue-300/30 hover:border-blue-400/50 transition-all duration-300">
                <div className="relative">
                  <input
                    ...
                    className="w-full ps-14 md:ps-16 pe-24 md:pe-28 py-4 text-sm md:text-base text-gray-900 placeholder:text-gray-400/70 placeholder:font-normal font-medium bg-transparent outline-none rounded-full relative z-10"
                ...
                {/* Right Side Elements */}
                <div className="absolute end-4 md:end-5 top-1/2 transform -translate-y-1/2 flex items-center gap-2 z-10">
                  {query && (
                    <button
                      type="button"
                      onClick={() => setQuery('')}
          ...
          {query && !showAIChat && (
            ...
            >
              {filteredDestinations.length} {t('search.results')}
            </motion.p>
          )}
        </div>
      </div>
      ...
      {filteredDestinations.length > 0 ? (
        ...
        >
          {query && !showAIChat ? (
            ...
            <p className="text-slate-500">{t('destinations.noDestinations')}</p>
          )}
        </motion.div>
      )}
