...
- 2026-05-20: Commit `3167938` made Netlify deploys optionally skip the prebuild Jest run. `netlify.toml` now sets `SKIP_TESTS=1` in the Netlify environment, and `package.json` guards the `prebuild` script so cached `.netlify` function artifacts stop colliding with Jest's haste map before the production build starts.
- 2026-05-19: Commit `efbc3b8` moved the public app-launch copy from 10 May to 30 May across `messages/{ar,de,en,es,fr,ru}.json`, keeping the tenant footer banner and `/coming-soon` QR-page copy aligned with the later release date.
- 2026-05-14: Commit `c4a30bc` fixed booking-status notifications. `app/admin/bookings/BookingsPageClient.tsx` now uses the saved normalized status returned by the API, `app/api/admin/bookings/[id]/route.ts` compares canonical status codes before sending mail, customer notifications now include tenant branding plus booking-reference/phone data, and `lib/email/emailService.ts` adds a new admin/operator status-change notification path so internal teams also get the update.
- 2026-04-30: Commit `379c575` moved the public app-launch copy from 30 April to 10 May across `messages/{ar,de,en,es,fr,ru}.json`, keeping the tenant footer banner and `/coming-soon` copy aligned with the later release date.
- 2026-04-29: Commit `495f4f1` added `app/admin/loading.tsx` and expanded `components/admin/Sidebar.tsx` with pending-route progress feedback, so brand-admin navigation now shows an explicit loading state instead of feeling stalled during page transitions.
- 2026-04-29: Commit `f60abdb` tightened tenant-aware destination/search behavior and admin titles. `app/admin/AdminDashboardClient.tsx`, `app/admin/bookings/BookingsPageClient.tsx`, and `components/admin/AdminDocumentTitle.tsx` now own the dashboard/bookings document-title flow, while `app/api/tenant/current/route.ts`, `lib/destinationPageSearch.ts`, `lib/tenantSiteSearch.ts`, `lib/translation/localizeTourCollection.ts`, `components/AISearchWidget.tsx`, and the destination/search routes further align public content discovery to the active tenant.
- 2026-04-29: A same-day commit sequence hardened tenant-scoped public navigation and discovery. `components/Footer.tsx` plus `lib/tenantNavigation.ts` now send generated fallback menu items to search pages instead of broken destination/category slugs, `lib/tenantSearchHitFilter.ts` with new tests keeps header and AI search hits scoped to the active tenant before rendering the dropdown, `app/api/tours/public/route.ts` now applies strict tenant filtering whenever a tenant is supplied, `app/api/interests/route.ts` rebuilds related interests from tenant-scoped categories plus attraction pages, and `app/[locale]/destinations/[slug]/DestinationPageClient.tsx` now guards its hero-image fallback path.
- 2026-04-26 (uncommitted): `app/admin/tours/ToursListClient.tsx` now makes the edit action explicit in both desktop rows and mobile cards. Tour titles are rendered as plain text instead of the only edit link, and a dedicated indigo `Edit` / `Edit tour` CTA now sits beside the existing action menu so brand-admin users can reach the edit form without relying on the title click target.
- 2026-04-24: `components/Footer.tsx` rebuilt the tenant footer's app-download banner around a generated QR code for the locale-aware `/coming-soon` landing page, with refreshed store buttons, richer mockup styling, and the scanned destination label shown inside the banner itself.
- 2026-04-24: `messages/{ar,de,en,es,fr,ru}.json` moved the public app-launch copy from 20 April to 30 April and now explicitly apologizes for the delay in the footer download messaging.
- 2026-04-23: `app/api/admin/bookings/route.ts` and `app/api/admin/reports/route.ts` now scope brand-admin data around the booking/tour tenant instead of broad joined-tour matches, reducing cross-tenant leakage in bookings and reporting. The same commit added `lib/tenantBranding.ts`, `lib/tenantNavigation.ts`, `lib/tenantSiteSearch.ts`, `lib/destinationPageSearch.ts`, and `lib/translation/localizeTourCollection.ts` so header/footer branding, destination search, live search, and localized tour collections stay tenant-aware across the public site.
- 2026-04-23: `lib/tenantPresentation.ts` and `lib/tenant.ts` now let German-clone tenant IDs inherit branding/theme from their source tenants instead of falling back to generic presentation data; coverage was added in `__tests__/lib/tenantPresentation.test.ts` and `__tests__/lib/tenantNavigation.test.ts`.
- 2026-04-18: Tour detail loading now prefetches stop-sale availability server-side via `lib/stopSaleFetcher.ts`, and `components/BookingSidebar.tsx` renders a full-calendar skeleton until client availability data is ready.
- 2026-04-17: Booking UI now disables stop-saled dates on the public calendar and surfaces unavailable-state messaging in `components/BookingSidebar.tsx`.
- 2026-04-17: Admin availability added a searchable tour picker in `app/admin/availability/page.tsx`, reducing friction when managing stop-sale windows across large tour catalogs.
- 2026-04-17: `app/[locale]/author/[slug]/page.tsx`, `lib/blogAuthors.ts`, and `scripts/seed-eeo-blog-posts.ts` expanded the editorial system with a richer public author page and 11 seeded team blog posts.
