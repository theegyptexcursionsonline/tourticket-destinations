  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 rounded-lg">
        ...
        <span className="text-xs font-medium text-gray-500 hidden sm:inline">Brand:</span>
      </div>
      ...
      <div className="relative">
        <select
          ...
          onChange={(e) => setSelectedTenantId(e.target.value)}
          disabled={isLoading}
          className="appearance-none bg-white border-2 border-gray-200 rounded-xl px-4 py-2 pr-10 text-sm font-semibold text-gray-800 hover:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-pointer transition-all disabled:opacity-50 disabled:cursor-not-allowed min-w-[180px]"
        >
          <option value={ALL_TENANTS_VALUE}>🌐 All Brands</option>
          {tenants.map((tenant) => (
            <option key={tenant.tenantId} value={tenant.tenantId}>
